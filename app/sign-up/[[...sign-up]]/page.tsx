...
export default function SignUpPage() {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      // Clerk may silently link an existing account — detect it here
              ...
              >
                <motion.div initial="hidden" animate="visible" className="flex flex-col gap-0">
                  <motion.div custom={0} variants={fadeUp} className="mb-8 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">Start reading smarter in minutes</p>
                  </motion.div>
                  ...
                  <motion.form custom={3} variants={fadeUp} onSubmit={handleRegister} className="flex flex-col gap-3">
                  ...
                  <motion.p custom={4} variants={fadeUp} className="mt-4 text-center text-[11px] text-muted-foreground/40">
              ...
              >
                <div className="mb-8 text-center">
                  <motion.div
                    ...
                    className="w-14 h-14 rounded-2xl bg-primary/10 border border-primary/20 flex items-center justify-center mx-auto mb-5"
                  >
                    <KeyRound className="w-6 h-6 text-primary" />
                  </motion.div>
                  <h1 className="text-2xl font-bold mb-1 tracking-tight">Check your email</h1>
                ...
                <form onSubmit={handleVerify} className="flex flex-col gap-3">
                  <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                    <Input
                      ...
                      autoFocus
                      className="h-14 text-center text-2xl tracking-[0.4em] font-mono bg-secondary/40 border-border/60 focus-visible:ring-primary/40 focus-visible:border-primary/50 transition-all"
                    />
                  </motion.div>
                  ...
                  <motion.button
                    ...
                    className="relative w-full h-10 rounded-lg bg-primary text-white text-sm font-medium overflow-hidden mt-1 transition-opacity hover:opacity-90 disabled:opacity-50"
                  >
                    <span className="relative z-10">{isLoading ? "Verifying…" : "Verify email"}</span>
                    <motion.span
                      className="absolute inset-0 bg-gradient-to-r from-transparent via-white/12 to-transparent -skew-x-12"
                      animate={{ x: ["-100%", "200%"] }}
                      transition={{ duration: 2, repeat: Infinity, repeatDelay: 3, ease: "easeInOut" }}
